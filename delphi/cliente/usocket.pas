...
type
  ...
  private
    ...
    procedure AtualizaConectados(pConectados : string);

    procedure HabilitaAba(IP : string);
  protected
...
uses uCadastroUsuario, uConfiguracao, uCriptografia, uUsuarios;
...
begin
  //2TipoMensagem
  //mmoConversa.Lines.Add(Socket.ReceiveText);
  lMensagemCrip   :=  Socket.ReceiveText;

  lcDescrip :=  TCriptografia.create;
  try
    mensagem := lcDescrip.DesCriptografaTexto(lMensagemCrip);
  finally
    FreeandNil(lcDescrip);
  end;

  lTipoMensagem := Copy(mensagem,1,13);
  Delete(mensagem,1,13);
  if (lTipoMensagem = '2TipoMensagem') then
    AtualizaConectados(mensagem)
  else
    begin
      mmoCrip.SelAttributes.Color := clred;
      mmoCrip.Lines.Add(lMensagemCrip);

      IpMensageiro := Copy(mensagem,1,Pos(':IP',mensagem)-1);
      IpMensageiro := StringReplace(IpMensageiro,frmConfiguracao.Porta+'*','',[rfReplaceAll]);
      HabilitaAba(IpMensageiro);
...
begin
 ...
 // meunro:=strtointdef(edtTexto.text,0);
  ...
  if not Empty(edtTexto.Text) then
    begin
      ClientUsuarios.IndexName := 'IdxUsuario';
...
begin
  ...
  if frmConfiguracao.leConfiguracao then
    begin
      ...
      Client.port   := StrToIntdef(frmConfiguracao.Porta,GetPorta);
      Client.Open;
//      try
//        Client.Active := True;
////        Client.Open;
...
begin
  try
    ...
    mmoCrip.SelAttributes.Color := clBlue;
    mmoCrip.Lines.Add(lcMensagemCrip);
...
begin
  if not Assigned(frmConfiguracao)then
    frmConfiguracao := TfrmConfiguracao.Create(self);
  frmConfiguracao.Reiniciar := FReiniciar;
...
var lConfiguracao : TIniFile;
begin
  lConfiguracao := TInifile.Create( ExtractFilePath(Application.ExeName) + 'Configuracao.ini' );
  try
    result := StrToIntDef(lConfiguracao.ReadString('Configuracao','Porta','8080'),8080);
  finally
...
begin
  if Assigned(frmUsuarios) then
    frmUsuarios.AjustarForm;
end;

procedure TfrmPrincipal.WMMove(var Msg: TMessage);
begin
  if Assigned(frmUsuarios) then
    frmUsuarios.AjustarForm;
end;

procedure TfrmPrincipal.AtualizaConectados(pConectados : string);
var lAux : string;
    i : Integer;
begin
  ClientUsuarios.Close;
  ClientUsuarios.CreateDataSet;
  while (pConectados <> '') do
    begin
      lAux := Copy(pConectados,1,Pos(',',pConectados));
      ClientUsuarios.Append;
      ClientUsuariosUsuario.AsString := Copy(lAux,1,Pos('-',lAux));
      Delete(lAux,1,Pos('-',lAux));
      ClientUsuariosIP.AsString      := Copy(lAux,1,Length(lAux) -1);
      ClientUsuarios.Post;
      Delete(pConectados,1,Pos(',',pConectados)+ 1);
    end;
end;

procedure TfrmPrincipal.HabilitaAba(IP : string);
var i, lIndex : Integer;
    lIP : string;
begin
  lIP := 'A' + StringReplace(IP,'.','',[rfReplaceAll]);
  lIndex := -1;
  for i := 0 to pgConversas.PageCount - 1 do
    begin
      if ((lIndex = -1) and (TTabSheet(pgConversas.Pages[i]).Name = lIP)) then
        lIndex := i;
    end;

  if (lIndex <> -1) then
    pgConversas.ActivePageIndex := lIndex
  else
    begin
      if ClientUsuarios.Locate('IP',IP,[]) then
        begin
          frmUsuarios.CriaTabShetUsuario;
        end;
    end;
end;
