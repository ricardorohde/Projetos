...
begin
  ...
  try
    if Assigned( TLookupEdit( self ).OnBeforeButton ) then
      TLookupEdit( self ).OnBeforeButton( Sender );
    
    frmPesquisa.grdPesquisa.DataSource := FListSource;

    frmPesquisa.Top  := TForm(Self.Owner).Top + Self.Top + (4 * Self.Height)-6;
    frmPesquisa.Left := TCustomForm(Self.Owner).Left + (2 * Self.Left)-2;
    frmPesquisa.Invalidate;
    ...
    begin
      ...
      if Assigned(DataSource) and (DataSource.DataSet.State in [dsInsert, dsEdit]) then
        if (DataField <> '') and (DataSource <> nil) then
        begin
          DataSource.DataSet.FieldByName(DataField).Value := lRetorno;

          if DataSource.DataSet.FieldByName(DataField) is TNumericField then
            Self.Text := Format(TNumericField(DataSource.DataSet.FieldByName(DataField)).DisplayFormat, [Self.Text]);
        end;
    end;
