...
uses
  ...
  DBCtrls, StrUtils, LookupEdit, uEditButton, DBEditBtn;
...
//    function RetornarTipo(Value: string): string;
    ...
    procedure AlimentarArrayListFieldsDBSearchEdit();
    procedure AlimentarArrayListFieldsDBEditBtn;
  public
    { Public declarations }
...
begin
  Result := '';
  if frmPesquisa.Owner is TDBSearchEdit then
    begin
      if (TDBSearchEdit(frmPesquisa.Owner).ListFields.Count <= 0) then
        Exit;

      if TDBSearchEdit(frmPesquisa.Owner).ListSource.DataSet.IsEmpty then
        Exit;
    end
  else
    begin
      if (TDBEditBtn(frmPesquisa.Owner).ListFields.Count <= 0) then
        Exit;

      if TDBEditBtn(frmPesquisa.Owner).ListSource.DataSet.IsEmpty then
        Exit;
    end;
...
begin
  ...
  begin
    ...
    begin
      if not lDataSet.FieldByName(FListFields[i].FieldName).IsNull then
...
procedure TfrmPesquisa.AlimentarArrayListFieldsDBSearchEdit();
var
  i       : Integer;
  lStrAux : string;
begin
  ...
  for i := 0 to TLookupEdit(Self.Owner).ListFields.Count -1 do
  begin
    lStrAux := TLookupEdit(Self.Owner).ListFields.Strings[i];
...
procedure TfrmPesquisa.AlimentarArrayListFieldsDBEditBtn();
var
  i       : Integer;
  lStrAux : string;
begin
  ...
  for i := 0 to TDBEditBtn(Self.Owner).ListFields.Count -1 do
  begin
    lStrAux := TDBEditBtn(Self.Owner).ListFields.Strings[i];
...
begin
  ...
  if frmPesquisa.Owner is TDBSearchEdit then
    AlimentarArrayListFieldsDBSearchEdit()
  else
    AlimentarArrayListFieldsDBEditBtn();
end;
