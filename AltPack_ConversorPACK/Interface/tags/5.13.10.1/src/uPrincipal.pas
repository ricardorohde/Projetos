...
a modificação de diversos pontos nas bpl dos concorrentes e também nas interfaces as quais são herdadas algumas funções.
}
...
type
  ...
  private
    ...
    procedure ImportaSistemaWPHD(pImportadorAlterdataBase : TImportadorAlterdataBase);
    procedure ImportaSistemaWDP(pImportadorAlterdataBase : TImportadorAlterdataBase);
    procedure ImportaSistemaWINSS(pImportadorAlterdataBase : TImportadorAlterdataBase);
    procedure ImportaSistemaWCONT(pImportadorAlterdataBase : TImportadorAlterdataBase);
  ...
  try
    ...
    try
      ...
      if chkWphd.Checked then
        ImportaSistemaWPHD(lImportadorAlterdataBase);

      {* WDP *}
      if chkWdp.Checked then
        ImportaSistemaWDP(lImportadorAlterdataBase);

      {* WCONT *}
      if chkWcont.Checked then
        ImportaSistemaWCONT(lImportadorAlterdataBase);
...
procedure TfrmImportador_Alterdata.ImportaSistemaWPHD(pImportadorAlterdataBase : TImportadorAlterdataBase);
...
begin
  ...
  //            end;
    ...
    lDataSetTemp := pImportadorAlterdataBase.GetDataSetImpConcorrente('WPHD.Empresa');
    if (lDataSetTemp <> nil) and (lDataSetTemp.RecordCount > 0) then
      lValidadorDeDados.Validar('WPHD.Empresa', FEmpresaList, lDataSetTemp, mmLog.Lines);
    lStatus.Position := lStatus.Position + 1;

     // Conversão da tabela 'WPhd.nx1'.
    if chkBureau.Checked then
      begin
        lDataSetTemp := pImportadorAlterdataBase.GetDataSetImpConcorrente('WPHD.Bureau');
    ...
    if chkSocio.Checked then
      begin
        lDataSetTemp := pImportadorAlterdataBase.GetDataSetImpConcorrente('WPHD.Socio');
        if (lDataSetTemp <> nil) and (lDataSetTemp.RecordCount > 0) then
...
//          lValidadorDeDados.Validar('WPHD.VinculoSocio', lDataSetTemp, mmLog.Lines);
    ...
    for i := 0 to Length(FEmpresaList) - 1 do
      begin
        ...
        lCdEmpresa  := Copy(FEmpresaList[i], 1, Pos('|', FEmpresaList[i]) - 1);
        ...
        if chkSocio.Checked then
          begin
            lDataSetTemp := pImportadorAlterdataBase.GetDataSetImpConcorrente('WPHD.VinculoSocio');
            if (lDataSetTemp <> nil) and (lDataSetTemp.RecordCount > 0) then
              lValidadorDeDados.Validar('WPHD.VinculoSocio', FEmpresaList, lCGCEmpresa, lCdEmpresa, lDataSetTemp, mmLog.Lines);
          end;
      end;
    ...
    for i := 0 to Length(FEmpresaList) - 1 do
      begin
        ...
        if chkSocio.Checked and chkWdp.Checked then
          begin
            lCdEmpresaOld := lValidadorDeDados.MapaEmpresa.ObterEmpresa(lCGCEmpresa, toeCPFCNPJ).CodigoAntigo;
            lDataSetTemp  := pImportadorAlterdataBase.GetDataSetImpConcorrente('WINSS.ContribuinteSocio', lCdEmpresaOld);
            if (lDataSetTemp <> nil) and (lDataSetTemp.RecordCount > 0) then
              lValidadorDeDados.Validar('WINSS.Contribuinte', FEmpresaList, lCGCEmpresa, lCdEmpresa, lDataSetTemp, mmLog.Lines);
          end;
...
procedure TfrmImportador_Alterdata.ImportaSistemaWDP(pImportadorAlterdataBase : TImportadorAlterdataBase);
var lValidadorDeDados : TValidadorDeDados;
    lStatus           : TStatus;
    lDataSetTemp      : TDataSet;
    lCGCEmpresa       : string;
...
begin
  ...
  try
    ...
    try
      ...
      lDataSetTemp := pImportadorAlterdataBase.GetDataSetImpConcorrente('WDP.Empresa');
      if lDataSetTemp <> nil then
        lValidadorDeDados.Validar('WDP.Empresa', FEmpresaList, lDataSetTemp, mmLog.Lines);
      lStatus.Position := lStatus.Position + 1;

      // Conversão da tabela 'Sind.nx1'.
      if chkSindicato.Checked then
        begin
          lDataSetTemp := pImportadorAlterdataBase.GetDataSetImpConcorrente('WDP.Sindicato');
          if (lDataSetTemp <> nil) and (lDataSetTemp.RecordCount > 0) then
      ...
      if chkHorario.Checked then
        begin
          lDataSetTemp := pImportadorAlterdataBase.GetDataSetImpConcorrente('WDP.Horario');
          if (lDataSetTemp <> nil) and (lDataSetTemp.RecordCount > 0) then
      ...
      if chkFuncao.Checked then
        begin
          lDataSetTemp := pImportadorAlterdataBase.GetDataSetImpConcorrente('WDP.Funcao');
          if (lDataSetTemp <> nil) and (lDataSetTemp.RecordCount > 0) then
      ...
      if chkBanco.Checked then
        begin
          lDataSetTemp := pImportadorAlterdataBase.GetDataSetImpConcorrente('WDP.Banco');
          if (lDataSetTemp <> nil) and (lDataSetTemp.RecordCount > 0) then
    ...
    try
      ...
      if chkAcumulado.Checked then
        begin
          lDataSetTemp := pImportadorAlterdataBase.GetDataSetImpConcorrente('WDP.Evento');
          if (lDataSetTemp <> nil) and (lDataSetTemp.RecordCount > 0) then
      ...
      if chkAfastamento.Checked then
        begin
          lDataSetTemp := pImportadorAlterdataBase.GetDataSetImpConcorrente('WDP.Especial');
          if (lDataSetTemp <> nil) and (lDataSetTemp.RecordCount > 0) then
      ...
      for i := 0 to Length(FEmpresaList) - 1 do
        begin
          ...
          if chkDepartamento.Checked then
            begin
              lDataSetTemp := pImportadorAlterdataBase.GetDataSetImpConcorrente('WDP.Departamento');
              if (lDataSetTemp <> nil) and (lDataSetTemp.RecordCount > 0) then
                lValidadorDeDados.Validar('WDP.Departamento', FEmpresaList, lCGCEmpresa, lCdEmpresa, lDataSetTemp, mmLog.Lines);
            end;

          // -> Conversão das tabelas 'F?????.nx1'.
          if chkFuncionario.Checked then
            begin
              lDataSetTemp  := pImportadorAlterdataBase.GetDataSetImpConcorrente('WDP.Funcionario', lCdEmpresaOld);
              if (lDataSetTemp <> nil) and (lDataSetTemp.RecordCount > 0) then
                lValidadorDeDados.Validar('WDP.Funcionario', FEmpresaList, lCGCEmpresa, lCdEmpresa, lDataSetTemp, mmLog.Lines)
              else
                Continue;
            end;
          ...
          if chkDependente.Checked then
            begin
              lDataSetTemp  := pImportadorAlterdataBase.GetDataSetImpConcorrente('WDP.Dependente', lCdEmpresaOld);
              if (lDataSetTemp <> nil) and (lDataSetTemp.RecordCount > 0) then
                lValidadorDeDados.Validar('WDP.Dependente', FEmpresaList, lCGCEmpresa, lCdEmpresa, lDataSetTemp, mmLog.Lines);
            end;

          // -> Conversão das tabelas 'A?????.nx1'.
          if chkAfastamento.Checked then
            begin
              lDataSetTemp := pImportadorAlterdataBase.GetDataSetImpConcorrente('WDP.Afastamento', lCdEmpresaOld);
              if (lDataSetTemp <> nil) and (lDataSetTemp.RecordCount > 0) then
                lValidadorDeDados.Validar('WDP.Afastamento', FEmpresaList, lCGCEmpresa, lCdEmpresa, lDataSetTemp, mmLog.Lines);

              lDataSetTemp := pImportadorAlterdataBase.GetDataSetImpConcorrente('WDP.Ferias', lCdEmpresaOld);
              if (lDataSetTemp <> nil) and (lDataSetTemp.RecordCount > 0) then
                lValidadorDeDados.Validar('WDP.Ferias', FEmpresaList, lCGCEmpresa, lCdEmpresa, lDataSetTemp, mmLog.Lines);
            end;

          // -> Conversão das tabelas 'R?????.nx1'.
          if chkAcumulado.Checked then
            begin
              lDataSetTemp := pImportadorAlterdataBase.GetDataSetImpConcorrente('WDP.Acumulado', lCdEmpresaOld);
              if (lDataSetTemp <> nil) and (lDataSetTemp.RecordCount > 0) then
                lValidadorDeDados.Validar('WDP.Acumulado', FEmpresaList, lCGCEmpresa, lCdEmpresa, lDataSetTemp, mmLog.Lines);
            end;

          // -> Conversão das tabelas 'L?????.nx1'.
          if chkReajusteSalarial.Checked then
            begin
              lDataSetTemp := pImportadorAlterdataBase.GetDataSetImpConcorrente('WDP.ReajusteSalarial', lCdEmpresaOld);
              if (lDataSetTemp <> nil) and (lDataSetTemp.RecordCount > 0) then
                lValidadorDeDados.Validar('WDP.ReajusteSalarial', FEmpresaList, lCGCEmpresa, lCdEmpresa, lDataSetTemp, mmLog.Lines);
            end;
  ...
  if chkWinss.Checked then
    ImportaSistemaWINSS(pImportadorAlterdataBase);

  DeleteFiles(GetEnvironmentVariable('TEMP') + '\*.CPack');
end;

procedure TfrmImportador_Alterdata.ImportaSistemaWINSS(pImportadorAlterdataBase : TImportadorAlterdataBase);
var lValidadorDeDados : TValidadorDeDados;
    lStatus           : TStatus;
    lDataSetTemp      : TDataSet;
    lCGCEmpresa       : string;
  ...
  try
    ...
    try
      ...
      if chkInssAcumulado.Checked then
        begin
          lDataSetTemp := pImportadorAlterdataBase.GetDataSetImpConcorrente('WINSS.Evento');
          if (lDataSetTemp <> nil) and (lDataSetTemp.RecordCount > 0) then
      ...
      if chkAutonomo.Checked then
        begin
          lDataSetTemp := pImportadorAlterdataBase.GetDataSetImpConcorrente('WINSS.Autonomo');
          if (lDataSetTemp <> nil) and (lDataSetTemp.RecordCount > 0) then
    ...
    try
      for i := 0 to Length(FEmpresaList) - 1 do
        begin
          ...
          if chkAutonomo.Checked then
            begin
              lDataSetTemp := pImportadorAlterdataBase.GetDataSetImpConcorrente('WINSS.ContribuinteAutonomo', lCdEmpresaOld);
              if (lDataSetTemp <> nil) and (lDataSetTemp.RecordCount > 0) then
                lValidadorDeDados.Validar('WINSS.Contribuinte', FEmpresaList, lCGCEmpresa, lCdEmpresa, lDataSetTemp, mmLog.Lines);
            end;
          ...
          if chkInssAcumulado.Checked then
            begin
              lDataSetTemp := pImportadorAlterdataBase.GetDataSetImpConcorrente('WINSS.Acumulado', lCdEmpresaOld);
              if (lDataSetTemp <> nil) and (lDataSetTemp.RecordCount > 0) then
                lValidadorDeDados.Validar('WINSS.Acumulado', FEmpresaList, lCGCEmpresa, lCdEmpresa, lDataSetTemp, mmLog.Lines);
            end;
...
procedure TfrmImportador_Alterdata.ImportaSistemaWCONT(pImportadorAlterdataBase : TImportadorAlterdataBase);
var lValidadorDeDados : TValidadorDeDados;
    lStatus           : TStatus;
    lDataSetTemp      : TDataSet;
    lCGCEmpresa       : string;
...
begin
  ...
  try
    ...
    try
      ...
      lDataSetTemp := pImportadorAlterdataBase.GetDataSetImpConcorrente('WCONT.Empresa');
      if lDataSetTemp <> nil then
        lValidadorDeDados.Validar('WCONT.Empresa', FEmpresaList, lDataSetTemp, mmLog.Lines);
      lStatus.Position := lStatus.Position + 1;

      // Conversão da tabela 'Hist.nx1'.
      if chkHistoricoPadrao.Checked then
        begin
          lDataSetTemp := pImportadorAlterdataBase.GetDataSetImpConcorrente('WCONT.HistoricoPadrao');
          if (lDataSetTemp <> nil) and (lDataSetTemp.RecordCount > 0) then
      ...
      if chkHistoricoPadrao.Checked then
        begin
          lDataSetTemp := pImportadorAlterdataBase.GetDataSetImpConcorrente('WCONT.LancamentoAutomatico');
          if (lDataSetTemp <> nil) and (lDataSetTemp.RecordCount > 0) then
      ...
      if chkPlanoDeConta.Checked then
        begin
          lDataSetTemp := pImportadorAlterdataBase.GetDataSetImpConcorrente('WCONT.PlanoDeConta');
          if (lDataSetTemp <> nil) and (lDataSetTemp.RecordCount > 0) then
            lValidadorDeDados.Validar('WCONT.PlanoDeConta', lDataSetTemp, mmLog.Lines);

          lDataSetTemp := pImportadorAlterdataBase.GetDataSetImpConcorrente('WCONT.Conta');
          if (lDataSetTemp <> nil) and (lDataSetTemp.RecordCount > 0) then
      ...
      if chkCentrodeCusto.Checked then
        begin
          lDataSetTemp := pImportadorAlterdataBase.GetDataSetImpConcorrente('WCONT.PlanoDeCentroDeCusto');
          if (lDataSetTemp <> nil) and (lDataSetTemp.RecordCount > 0) then
            lValidadorDeDados.Validar('WCONT.PlanoDeCentroDeCusto', lDataSetTemp, mmLog.Lines);

          lDataSetTemp := pImportadorAlterdataBase.GetDataSetImpConcorrente('WCONT.CentroDeCusto');
          if (lDataSetTemp <> nil) and (lDataSetTemp.RecordCount > 0) then
      ...
      for i := 0 to Length(FEmpresaList) - 1 do
        begin
          ...
          if chkLancamentoTemporario.Checked then
            begin
              lCdEmpresaOld := lValidadorDeDados.MapaEmpresa.ObterEmpresa(lCGCEmpresa, toeCPFCNPJ).CodigoAntigo;
              lDataSetTemp := pImportadorAlterdataBase.GetDataSetImpConcorrente('WCONT.LancamentoTemporario', lCdEmpresaOld);
              if (lDataSetTemp <> nil) and (lDataSetTemp.RecordCount > 0) then
                lValidadorDeDados.Validar('WCONT.LancamentoTemporario', FEmpresaList, lCGCEmpresa, lCdEmpresa, lDataSetTemp, mmLog.Lines);
            end;
