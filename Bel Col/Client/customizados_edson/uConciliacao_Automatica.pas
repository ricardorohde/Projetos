    ...
    begin
      ...
      if (cdsMovimentos_Arquivo.IsEmpty) or (not cdsMovimentos_Arquivo.IsEmpty and (cdsMovimentos_ArquivoINCLUIDO.AsInteger = 1)) then
        begin
          ...
          cdsEfetiva_MovimentosTIPOMOVIMENTO.AsString := cdsMovimentos_BancariosTIPO.AsString;
          ...
          begin
            ...
            begin
              case AnsiIndexStr( AnsiUpperCase(cdsMovimentos_Arquivo.Fields[i].FieldName),
                                 ['MOVIMENTO', 'TIPOMOVIMENTO', 'VALOR', 'HISTORICO', 'DOCUMENTO']) of
                0: lbIsEquals:= cdsMovimentos_Arquivo.Fields[i].AsDateTime = cdsMovimentos_BancariosMOVIMENTACAO.AsDateTime;
                1:
                  begin
                    ...
                    lbIsEquals:= Trim(cdsMovimentos_Arquivo.Fields[i].AsString) = lsTipoMov;
                  end;
                2: lbIsEquals:= cdsMovimentos_Arquivo.Fields[i].AsFloat = cdsMovimentos_BancariosVALOR.AsFloat;
                3: lbIsEquals:= cdsMovimentos_Arquivo.Fields[i].AsString = cdsMovimentos_BancariosHISTORICO.AsString;
                4: lbIsEquals:= cdsMovimentos_Arquivo.Fields[i].AsString = cdsMovimentos_BancariosDOCUMENTO.AsString;
              end;

              if not lbIsEquals then
                Break;
            end;
