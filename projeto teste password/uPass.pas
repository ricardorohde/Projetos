...
var i : Integer;
begin
  for i := Low(PisEmpresa205para231) to (High(PisEmpresa205para231)) do
    begin
      ProcessaEmpresa(PisEmpresa205para231[i],205,231);
      //ProcessaEmpresa(PisEmpresa205para231[i],231,205);
    end;

  for i := Low(PisEmpresa227para236) to (High(PisEmpresa227para236)) do
    begin
      ProcessaEmpresa(PisEmpresa227para236[i],227,236);
      //ProcessaEmpresa(PisEmpresa227para236[i],236,227);
    end;

  for i := Low(PisEmpresa227para237) to High(PisEmpresa227para237) do
    begin
      ProcessaEmpresa(PisEmpresa227para237[i],227,237);
      //ProcessaEmpresa(PisEmpresa227para237[i],237,227);
    end;

  for i := Low(PisEmpresa229para230) to High(PisEmpresa229para230) do
    begin
      ProcessaEmpresa(PisEmpresa229para230[i],229,230);
      //ProcessaEmpresa(PisEmpresa229para230[i],230,229);
    end;
end;
...
//    lQryFuncionario.sql.add('where NrPisPasep = '+ QuotedStr(pis));
    ...
    if not lQryFuncionario.IsEmpty then
      begin
        lQryFuncionario.First;
        while not lQryFuncionario.Eof do
          begin
...
//            ProcessaInsercaoJustific(lQryFuncionario.Fields.fieldByName('IdFuncionarioold').AsString,
//                                   lQryFuncionario.Fields.fieldByName('IdFuncionarionew').AsString,
//                                   Empresa_de,Empresa_Para,'Justific');
//            ProcessaInsercaoSMarca(lQryFuncionario.Fields.fieldByName('IdFuncionarioold').AsString,
//                                   lQryFuncionario.Fields.fieldByName('IdFuncionarionew').AsString,
//                                   Empresa_de,Empresa_Para,'SMarca');
//            ProcessaInsercaoVisita(lQryFuncionario.Fields.fieldByName('IdFuncionarioold').AsString,
//                                   lQryFuncionario.Fields.fieldByName('IdFuncionarionew').AsString,
//                                   Empresa_de,Empresa_Para,'Visita');
//            ProcessaInsercaoJustificLegal(lQryFuncionario.Fields.fieldByName('IdFuncionarioold').AsString,
//                                          lQryFuncionario.Fields.fieldByName('IdFuncionarionew').AsString,
//                                          Empresa_de,Empresa_Para,'JustificLegal');


            //horario
            ProcessaInsercaoHorario(lQryFuncionario.Fields.fieldByName('IdFuncionarioold').AsString,
                                    lQryFuncionario.Fields.fieldByName('IdFuncionarionew').AsString,
                                    Empresa_de,Empresa_Para,'HrComp','wponto');

            ProcessaInsercaoHorario(lQryFuncionario.Fields.fieldByName('IdFuncionarioold').AsString,
                                    lQryFuncionario.Fields.fieldByName('IdFuncionarionew').AsString,
                                    Empresa_de,Empresa_Para,'HrTurno','wponto');

            ProcessaInsercaoHorario(lQryFuncionario.Fields.fieldByName('IdFuncionarioold').AsString,
                                    lQryFuncionario.Fields.fieldByName('IdFuncionarionew').AsString,
                                    Empresa_de,Empresa_Para,'EscalaRevez','wponto');

            ProcessaInsercaoHorario(lQryFuncionario.Fields.fieldByName('IdFuncionarioold').AsString,
                                    lQryFuncionario.Fields.fieldByName('IdFuncionarionew').AsString,
                                    Empresa_de,Empresa_Para,'EscalaRevezItem','wponto');

            ProcessaInsercaoHorario(lQryFuncionario.Fields.fieldByName('IdFuncionarioold').AsString,
                                    lQryFuncionario.Fields.fieldByName('IdFuncionarionew').AsString,
                                    Empresa_de,Empresa_Para,'HistHor','wponto');

            lQryFuncionario.Next;
          end;
      end;
  finally
...
var lQrySeleciona : TSetupQuery;
  ...
  begin
    ...
    lQryInsere.sql.add('and  IdFuncionario = '+ QuotedStr(IdFuncionarioNew));
        ...
        for i := 0 to lQrySeleciona.FieldCount - 1 do
          begin
            ...
            if (lQrySeleciona.Fields[i].DataType = ftAutoInc) then
              continue;
            // Ser for autoinc continua o for
            if ((lQrySeleciona.Fields[i].DataType = ftInteger) or
                (lQrySeleciona.Fields[i].DataType = ftstring)) and
               (lQrySeleciona.Fields[i].AutoGenerateValue = arAutoInc) then
               continue;

            if ((lQrySeleciona.Fields[i].FieldName = 'CdEmpresa') or
                (lQrySeleciona.Fields[i].FieldName = 'IdFuncionario')) then
              begin
                ...
                else if (lQrySeleciona.Fields[i].FieldName = 'IdFuncionario') then
                  lQryInsere.Fields.FieldByName(lQrySeleciona.Fields[i].FieldName).Value := IdFuncionarioNew;
              end
            else
...
begin
  ...
  try
    ...
    lQrySeleciona.sql.add('where CdEmpresa = '+ IntToStr(Empresa_de));
    lQrySeleciona.sql.add('and  IdFuncionario = '+ QuotedStr(IdFuncionarioold));
    ...
    lQrySeleciona.sql.add('and DtMovimentacao <= {date '+ QuotedSTr(FormatDateTime('yyyy-mm-dd',StrToDate('13/06/2012')))+ '}');
    lQrySeleciona.Open;
...
procedure TForm1.ProcessaInsercaoJustific(IdFuncionarioold,
  IdFuncionarioNew: string; Empresa_de, Empresa_Para: Integer;
  NmTabela: string);
var lQrySeleciona : TSetupQuery;
    lQryInsere : TSetupQuery;
    i : integer;
        ...
        try
          ...
          while not lQrySeleciona.Eof do
            begin
              lQryInsere.Close;
              lQryInsere.sql.Clear;
              lQryInsere.sql.add('SELECT IdJustificativa,');
              lQryInsere.sql.add('CdEmpresa,');
              lQryInsere.sql.add('IdFuncionario,');
              lQryInsere.sql.add('TpPessoa,');
              lQryInsere.sql.add('DtMovimentacao,');
              lQryInsere.sql.add('IdMotivo,');
              lQryInsere.sql.add('NrMinutosAtraso,');
              lQryInsere.sql.add('NrMinutosHoraExtra,');
              lQryInsere.sql.add('StAbono,');
              lQryInsere.sql.add('StWDP,');
              lQryInsere.sql.add('DsObservacao,');
              lQryInsere.sql.add('StFalta,');
              lQryInsere.sql.add('DtUltimaAtualizacao');
              lQryInsere.sql.add('from wponto.Justific');
              lQryInsere.sql.add('where CdEmpresa = '+ IntToStr(Empresa_Para));
              lQryInsere.sql.add('and  IdFuncionario = '+ QuotedStr(IdFuncionarioNew));
              lQryInsere.sql.add('and  TpPessoa = ''F''');
              lQryInsere.sql.add('and DtMovimentacao = {date '+ QuotedStr(FormatDateTime('yyyy-mm-dd',lQrySeleciona.Fields.fieldByName('DtMovimentacao').AsDateTime))+ '}');
              ...
              if lQryInsere.IsEmpty then
                begin
                  lQryInsere.Append;
                  lQryInsere.Fields.FieldByName('CdEmpresa').asinteger            := Empresa_Para;
                  lQryInsere.Fields.FieldByName('IdFuncionario').AsString         := IdFuncionarioNew;
                  lQryInsere.Fields.FieldByName('IdJustificativa').AsString       := NewId;
                  lQryInsere.Fields.FieldByName('TpPessoa').AsString              := lQrySeleciona.Fields.FieldByName('TpPessoa').AsString;
                  lQryInsere.Fields.FieldByName('DtMovimentacao').AsdateTime      := lQrySeleciona.Fields.FieldByName('DtMovimentacao').AsdateTime;
                  lQryInsere.Fields.FieldByName('IdMotivo').AsString              := lQrySeleciona.Fields.FieldByName('IdMotivo').AsString;
                  lQryInsere.Fields.FieldByName('NrMinutosAtraso').asinteger      := lQrySeleciona.Fields.FieldByName('NrMinutosAtraso').asinteger;
                  lQryInsere.Fields.FieldByName('NrMinutosHoraExtra').asinteger   := lQrySeleciona.Fields.FieldByName('NrMinutosHoraExtra').asinteger;
                  lQryInsere.Fields.FieldByName('StAbono').AsString               := lQrySeleciona.Fields.FieldByName('StAbono').AsString;
                  lQryInsere.Fields.FieldByName('StWDP').AsString                 := lQrySeleciona.Fields.FieldByName('StWDP').AsString;
                  lQryInsere.Fields.FieldByName('DsObservacao').Value             := lQrySeleciona.Fields.FieldByName('DsObservacao').Value;
                  lQryInsere.Fields.FieldByName('StFalta').AsString               := lQrySeleciona.Fields.FieldByName('StFalta').AsString;
                  lQryInsere.Fields.FieldByName('DtUltimaAtualizacao').asdateTime := lQrySeleciona.Fields.FieldByName('DtUltimaAtualizacao').asdateTime;
                  lQryInsere.Post;
                end;
              //lQrySeleciona.Delete;
              lQrySeleciona.Next;
...
procedure TForm1.ProcessaInsercaoJustificLegal(IdFuncionarioold,
  IdFuncionarioNew: string; Empresa_de, Empresa_Para: Integer;
  NmTabela: string);
var lQrySeleciona : TSetupQuery;
    lQryInsere : TSetupQuery;
        ...
        try
          ...
          while not lQrySeleciona.Eof do
            begin
              lQryInsere.Close;
              lQryInsere.sql.Clear;
              lQryInsere.sql.add('SELECT IdJustificativaLegal,');
              lQryInsere.sql.add('CdEmpresa,');
              lQryInsere.sql.add('IdFuncionario,');
              lQryInsere.sql.add('TpPessoa,');
              lQryInsere.sql.add('DtMovimentacao,');
              ...
              lQryInsere.sql.add('StFalta,');
              lQryInsere.sql.add('DtUltimaAtualizacao');
              lQryInsere.sql.add('from wponto.JustificLegal');
              lQryInsere.sql.add('where CdEmpresa = '+ IntToStr(Empresa_Para));
              lQryInsere.sql.add('and  IdFuncionario = '+ QuotedStr(IdFuncionarioNew));
              lQryInsere.sql.add('and  TpPessoa = ''F''');
              lQryInsere.sql.add('and DtMovimentacao = {date '+ QuotedStr(FormatDateTime('yyyy-mm-dd',lQrySeleciona.Fields.fieldByName('DtMovimentacao').AsDateTime))+ '}');
              ...
              if lQryInsere.IsEmpty then
                begin
                  ...
                  lQryInsere.Fields.FieldByName('CdEmpresa').asinteger            := Empresa_Para;
                  lQryInsere.Fields.FieldByName('IdFuncionario').AsString         := IdFuncionarioNew;
                  lQryInsere.Fields.FieldByName('IdJustificativaLegal').AsString  := NewId;
                  lQryInsere.Fields.FieldByName('TpPessoa').AsString              := lQrySeleciona.Fields.FieldByName('TpPessoa').AsString;
                  lQryInsere.Fields.FieldByName('DtMovimentacao').AsdateTime      := lQrySeleciona.Fields.FieldByName('DtMovimentacao').AsdateTime;
                  lQryInsere.Fields.FieldByName('IdMotivo').AsString              := lQrySeleciona.Fields.FieldByName('IdMotivo').AsString;
                  lQryInsere.Fields.FieldByName('NrMinutosAtraso').asinteger      := lQrySeleciona.Fields.FieldByName('NrMinutosAtraso').asinteger;
...
procedure TForm1.ProcessaInsercaoSMarca(IdFuncionarioold,
  IdFuncionarioNew: string; Empresa_de, Empresa_Para: Integer;
  NmTabela: string);
var lQrySeleciona : TSetupQuery;
    lQryInsere : TSetupQuery;
        ...
        try
          ...
          while not lQrySeleciona.Eof do
            begin
              lQryInsere.Close;
              lQryInsere.sql.Clear;
              lQryInsere.sql.add('SELECT IdSemMarcacao,');
              lQryInsere.sql.add('CdEmpresa,');
              lQryInsere.sql.add('IdFuncionario,');
              lQryInsere.sql.add('TpPessoa,');
              lQryInsere.sql.add('DtMovimentacao,');
              lQryInsere.sql.add('NrMinutos,');
              lQryInsere.sql.add('StAbono,');
              lQryInsere.sql.add('StWDP,');
              lQryInsere.sql.add('HrSemMarcacao,');
              lQryInsere.sql.add('DtUltimaAtualizacao');
              lQryInsere.sql.add('from wponto.SMarca');
              lQryInsere.sql.add('where CdEmpresa = '+ IntToStr(Empresa_Para));
              lQryInsere.sql.add('and  IdFuncionario = '+ QuotedStr(IdFuncionarioNew));
              lQryInsere.sql.add('and  TpPessoa = ''F''');
              lQryInsere.sql.add('and DtMovimentacao = {date '+ QuotedStr(FormatDateTime('yyyy-mm-dd',lQrySeleciona.Fields.fieldByName('DtMovimentacao').AsDateTime))+ '}');
              ...
              if lQryInsere.IsEmpty then
                begin
                  ...
                  lQryInsere.Fields.FieldByName('CdEmpresa').asinteger            := Empresa_Para;
                  lQryInsere.Fields.FieldByName('IdFuncionario').AsString         := IdFuncionarioNew;
                  lQryInsere.Fields.FieldByName('IdSemMarcacao').AsString         := NewId;
                  lQryInsere.Fields.FieldByName('TpPessoa').AsString              := lQrySeleciona.Fields.FieldByName('TpPessoa').AsString;
                  lQryInsere.Fields.FieldByName('DtMovimentacao').AsDateTime      := lQrySeleciona.Fields.FieldByName('DtMovimentacao').AsDateTime;
                  lQryInsere.Fields.FieldByName('NrMinutos').AsInteger            := lQrySeleciona.Fields.FieldByName('NrMinutos').AsInteger;
                  lQryInsere.Fields.FieldByName('StAbono').AsString               := lQrySeleciona.Fields.FieldByName('StAbono').AsString;
                  lQryInsere.Fields.FieldByName('StWDP').AsString                 := lQrySeleciona.Fields.FieldByName('StWDP').AsString;
                  lQryInsere.Fields.FieldByName('HrSemMarcacao').AsString         := lQrySeleciona.Fields.FieldByName('HrSemMarcacao').AsString;
                  lQryInsere.Fields.FieldByName('DtUltimaAtualizacao').AsDateTime := lQrySeleciona.Fields.FieldByName('DtUltimaAtualizacao').AsDateTime;
                  lQryInsere.Post;
                end;
              //lQrySeleciona.Delete;
...
procedure TForm1.ProcessaInsercaoVisita(IdFuncionarioold,
  IdFuncionarioNew: string; Empresa_de, Empresa_Para: Integer;
  NmTabela: string);
var lQrySeleciona : TSetupQuery;
    lQryInsere : TSetupQuery;
        ...
        try
          ...
          while not lQrySeleciona.Eof do
            begin
              ...
              lQryInsere.sql.Clear;
              lQryInsere.sql.add('SELECT *');
              lQryInsere.sql.add('from wponto.VISITA');
              lQryInsere.sql.add('where CdEmpresa = '+ IntToStr(Empresa_Para));
              lQryInsere.sql.add('and  IdFuncionario = '+ QuotedStr(IdFuncionarioNew));
              lQryInsere.sql.add('and  TpPessoa = ''F''');
              lQryInsere.sql.add('and DtMovimentacao = {date '+ QuotedStr(FormatDateTime('yyyy-mm-dd',lQrySeleciona.Fields.fieldByName('DtMovimentacao').AsDateTime))+ '}');
              ...
              if lQryInsere.IsEmpty then
                begin
                  ...
                  lQryInsere.Fields.FieldByName('CdEmpresa').asinteger               := Empresa_Para;
                  lQryInsere.Fields.FieldByName('IdFuncionario').AsString            := IdFuncionarioNew;
                  lQryInsere.Fields.FieldByName('HrInicio').AsString                 := lQrySeleciona.Fields.FieldByName('HrInicio').AsString;
                  lQryInsere.Fields.FieldByName('HrFim').AsString                    := lQrySeleciona.Fields.FieldByName('HrFim').AsString;
                  lQryInsere.Fields.FieldByName('TpPessoa').AsString                 := lQrySeleciona.Fields.FieldByName('TpPessoa').AsString;
                  lQryInsere.Fields.FieldByName('DtMovimentacao').AsDateTime         := lQrySeleciona.Fields.FieldByName('DtMovimentacao').AsDateTime;
                  lQryInsere.Fields.FieldByName('NrMinutosHoraExtra').AsInteger      := lQrySeleciona.Fields.FieldByName('NrMinutosHoraExtra').AsInteger;
                  lQryInsere.Fields.FieldByName('NrMinutosHoraExtraLegal').AsInteger := lQrySeleciona.Fields.FieldByName('NrMinutosHoraExtraLegal').AsInteger;
                  lQryInsere.Fields.FieldByName('NrQntMinAlmoco').AsInteger          := lQrySeleciona.Fields.FieldByName('NrQntMinAlmoco').AsInteger;
                  lQryInsere.Fields.FieldByName('StWDP').AsString                    := lQrySeleciona.Fields.FieldByName('StWDP').AsString;
                  lQryInsere.Fields.FieldByName('DtUltimaAtualizacao').AsDateTime    := lQrySeleciona.Fields.FieldByName('DtUltimaAtualizacao').AsDateTime;
                  lQryInsere.Post;
                end;
...
procedure TForm1.ProcessaInsercaoHorario(IdFuncionarioold,
  IdFuncionarioNew: string; Empresa_de, Empresa_Para: Integer;
  NmTabela,schema: string);
var lQrySeleciona : TSetupQuery;
    lQryInsere : TSetupQuery;
    i : integer;
  ...
  begin
    ...
    lQryInsere.sql.Clear;
    lQryInsere.sql.add('select * ');
    lQryInsere.sql.add('from ' + schema + '.'+ NmTabela);
    lQryInsere.sql.add('where CdEmpresa = '+ IntToStr(Empresa_Para));
    lQryInsere.sql.add('and  IdFuncionario = '+ QuotedStr(IdFuncionarioNew));
    lQryInsere.sql.add('and  TpPessoa = ''F''');

    case  AnsiIndexStr(NmTabela, ['HrComp', 'HistHor','HrTurno','EscalaRevez','EscalaRevezItem'])  of
      0 :
        begin
          lQryInsere.sql.add('and  NrDiaSemana = ' + lQrySeleciona.Fields.FieldByName('NrDiaSemana').AsString);
          lQryInsere.sql.add('and  DtAlteracao = {Date ' + QuotedStr(FormatDateTime('yyyy-mm-dd',lQrySeleciona.Fields.FieldByName('DtAlteracao').AsDateTime))+ '}');
        end;

      1 :
        begin
          lQryInsere.sql.add('and  IdHorario = ' + QuotedStr(lQrySeleciona.Fields.FieldByName('IdHorario').AsString));
          lQryInsere.sql.add('and  DtAlteracao = {Date ' + QuotedStr(FormatDateTime('yyyy-mm-dd',lQrySeleciona.Fields.FieldByName('DtAlteracao').AsDateTime))+ '}');
        end;

      2 :
        begin
          lQryInsere.sql.add('and  DsTurno = ' + QuotedStr(lQrySeleciona.Fields.FieldByName('DsTurno').AsString));
          lQryInsere.sql.add('and  DtAlteracao = {Date ' + QuotedStr(FormatDateTime('yyyy-mm-dd',lQrySeleciona.Fields.FieldByName('DtAlteracao').AsDateTime))+ '}');
        end;

      3 :
        begin
          lQryInsere.sql.add('and  DsReferencia = ' + QuotedStr(lQrySeleciona.Fields.FieldByName('DsReferencia').AsString));
        end;
      4 :
        begin
          lQryInsere.sql.add('and  NrDia = ' + lQrySeleciona.Fields.FieldByName('NrDia').AsString);
          lQryInsere.sql.add('and  DsReferencia = ' + QuotedStr(lQrySeleciona.Fields.FieldByName('DsReferencia').AsString));
        end;
    end;
    lQryInsere.Open;
...
begin
  ...
  try
    lQrySeleciona.sql.add('select * ');
    lQrySeleciona.sql.add('from '+ schema + '.'+ NmTabela);
    lQrySeleciona.sql.add('where CdEmpresa = '+ IntToStr(Empresa_de));
    lQrySeleciona.sql.add('and  IdFuncionario = '+ QuotedStr(IdFuncionarioold));
    lQrySeleciona.sql.add('and  TpPessoa = ''F''');
    lQrySeleciona.Open;
