...
uses
  ...
  ImgList, Db, DBTables,dbctrls, MemDS, DBAccess, Uni, ADODB;
type
  ...
  TdmCOM002A = class(TDataModule)
    qyPrograma_Operador: TADOQuery;
    ImIcones: TImageList;
    qyTmp: TADOQuery;
    dsIcone: TDataSource;
    qyPrograma_Grupo: TADOQuery;
    qyAtalho: TADOQuery;
...
begin
  ...
  begin
     ...
     begin
        close;
        //Parameters.ParamByName('CDOPERADOR').asString := AppSysOper;
        Parameters.ParamByName('CDOPERADOR').DataType := ftString;
        Parameters.ParamByName('CDOPERADOR').Value    := AppSysOper;
        open;
        while not eof do
        begin
...
//        ParamByName('CDOPERADOR').asString := AppSysOper;
        Parameters.ParamByName('CDGRUPO').DataType := ftInteger;
        Parameters.ParamByName('CDGRUPO').Value    := AppSysGrupo;
        Parameters.ParamByName('CDOPERADOR').DataType := ftString;
        Parameters.ParamByName('CDOPERADOR').Value := AppSysOper;
        open;
        while not eof do
